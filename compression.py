...
def test_greyscale(file_name):
	...
	total_ele=0
	for t in transforms:
		total_ele=total_ele+len(t)
	compressed_size=total_ele*32
	orginal_size=img.shape[0]*img.shape[1]*8
	
	iterations = decompress(transforms, 8, 4, 8)
	print "CR:",orginal_size/compressed_size,"Time:",elapsed,"Image size in bytes",img.shape[0]*img.shape[1]*8
	print "PSNR",measure.compare_psnr(img,iterations[-1],img.max()-img.min())
	print  "SIM",measure.compare_ssim(img,iterations[-1])
